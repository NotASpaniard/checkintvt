...
import { Box, Page, Text, Icon, Spinner } from "zmp-ui";
...
function StatsPage() {
  ...
  useEffect(() => {
    getUserInfo({
      success: (data) => {
        fetchStats(data.userInfo.id);
      },
      fail: (err) => {
        console.error("Loi lay thong tin Zalo trong Stats:", err);
        setLoading(false);
      }
    });
  }, []);

  const fetchStats = async (zaloId) => {
    try {
      const res = await fetch(`${ API_BASE } /api/user / stats ? zalo_id = ${ zaloId } `);
    ...
    return (
      ...
      {loading ? (
        <Box flex justifyContent="center" alignItems="center" p={10}>
          <Spinner />
          <Text ml={2} style={{ color: "white" }}>Đang tính toán dữ liệu...</Text>
        </Box>
      ) : (
        <>
          ...
          <Box p={4} m={4} style={{ background: "rgba(10, 11, 16, 0.8)", borderRadius: "12px", border: "1px solid rgba(255, 255, 255, 0.1)" }}>
            ...
            <Text style={{ color: "#a0aec0", fontSize: "14px", lineHeight: "1.5" }}>
              Hệ thống ghi nhận thời gian điểm danh đầu tiên trong ngày làm giờ đến. Vui lòng đứng trước camera trước 08:30 AM để không bị tính là đi trễ.
            </Text>
          </Box>
        </>
      )}
